...
Niivue provides powerful tools for manual and semi-automatic segmentation of volumetric medical images. These tools allow users to create, edit, and save regions of interest (ROIs) directly within the visualization. Whether you need precise manual delineation of structures or want to leverage automatic segmentation methods, Niivue's drawing tools offer flexibility and control.
...
Drawing in Niivue allows you to create and modify segmentations by painting directly on the image slices. The drawing layer is a separate volume that overlays the base image, with each voxel value representing a different label or region.
...
The Grow Cut algorithm is an interactive segmentation method that expands from seed regions:
...
const nv = new Niivue({
  ...
  clickToSegmentIs2D: true,      // Confine to 2D slice (false for 3D)
...
When enabled, clicking on a voxel will automatically segment connected voxels with similar intensity values.
...
Drawing and segmentation tools are essential for many neuroimaging and medical imaging workflows. Niivue provides a flexible and comprehensive set of tools that can be integrated into web applications for use in research, clinical, or educational settings. These tools enable precise definition of regions of interest for further analysis, visualization, or quantification. 