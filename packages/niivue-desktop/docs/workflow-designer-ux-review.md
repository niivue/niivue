...
23. (P2) ~~"Load All in Viewer" gated on `>1 file`; entire row should be clickable.~~ DONE — `DicomImportDialog` rows are now full-width `<button>`s with hover/focus rings; clicking anywhere on the filename triggers load. The footer "Load All in Viewer" stays gated on `>1 file` (it would be redundant with the single row otherwise).
24. (P1) ~~Serial preview rendering — 50 files = 30+ seconds.~~ DONE — `StepBidsPreview` now uses an IntersectionObserver-driven `LazyThumbnail` component backed by a `useLazySeriesPreviews` queue: thumbnails are only rendered once their row scrolls into view (with a 200px pre-warm margin), and a single worker still serializes the actual WebGL render so we never hold two GL contexts at once. The first viewport (~5-7 thumbnails) populates in seconds instead of staring at 50 black squares for half a minute, and paths the user never scrolls past are never rendered at all.
...
12. (P1) ~~Icon size drift across 12/14/15/20/32px.~~ INTENTIONAL — sizes are tuned per container: 9-11px glyphs inside 14-18px circular badges (legend swatches, edge-midpoint warnings, delete buttons), 12px for the palette search field, 20px (w-5) for header chrome, 24px for hero glyphs inside 48px circles, 32px (w-8) for the gallery's largest hero. Each ratio is ~60-70% of its container so the glyph reads at a glance.
...
- **A11Y-4 (P0)** ~~PARTIAL~~ DONE — popover trigger now carries a verbose `aria-label` and an `aria-live="polite"` visually-hidden region announces the issue counts when they change. Per-field association is now wired in `ContextSpineDesigner`: each input row looks up `validation.errors.find(e => e.stepName === step.name && e.field === inp.name)` (falling back to a matching warning) and renders the message as a `role="alert"` / `role="status"` Text below the row. The Select.Trigger picks up `aria-invalid` and `aria-describedby={fieldErrorId}` so screen readers announce the issue when focus enters the input.
- **A11Y-5 (P1)** ~~Diagram not keyboard-operable — nodes not focusable, no SR fallback for wiring.~~ DONE — `StepNode` carries `tabIndex={0}`, `role="button"`, an `aria-label` that names the step + error/warning state, and Enter/Space activates `onSelect`. Edge deletion has a focusable Cross1Icon at the midpoint with its own focus ring (`workflow-edge-delete` in `main.css`). SR fallback for wiring is now provided via a visually-hidden `<span>` (referenced by `aria-describedby`) that summarizes each input's binding source: "Inputs: T1 from step skull_strip output brain; mask constant; output_dir unbound." Edge-graph keyboard *navigation* (Tab between edges) is still managed by React Flow's defaults — the SR description means a keyboard-only user can understand the wiring from each node without depending on edge focus.
