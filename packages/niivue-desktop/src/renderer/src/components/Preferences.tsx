...
export const Preferences: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const [modifiedPrefs, setModifiedPrefs] = useState<Record<string, ConfigValue>>({})
  ...
  const renderControl = (key: string, meta: any): JSX.Element => {
        ...
        return (
          <Switch
            ...
            onCheckedChange={(checked) =>
              updateOption(key, checked, meta.requiresDraw, meta.requiresUpdateGLVolume)
            }
          />
        )
      case 'slider': {
      ...
      case 'text': {
        ...
        return (
          <input
            ...
            onBlur={() =>
              updateOption(key, localValue, meta.requiresDraw, meta.requiresUpdateGLVolume)
            }
          />
        )
      }
        ...
        return (
          <ColorPicker
            ...
            onChange={(e) =>
              updateOption(
                key,
                hexToRgba10(e.target.value),
                meta.requiresDraw,
                meta.requiresUpdateGLVolume
              )
            }
          />
        )
