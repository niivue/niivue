...
export const LabelManagerDialog = ({
  ...
  const editLabel = (label: NVLabel3D): void => {
    if (!nv) return
    setEditMode(true)
    setText(label.text)
  ...
  const updateLabel = (): void => {
    ...
    if (label) {
      label.onClick = (clickedLabel: NVLabel3D, e?: MouseEvent): void => {
        if (e?.button === 2) {
          e.preventDefault()
          editLabel(clickedLabel)
        } else {
          const pt = Array.isArray(clickedLabel.points?.[0])
            ? clickedLabel.points[0]
            : clickedLabel.points
        ...
        <Dialog.Content className="bg-white rounded shadow-lg fixed z-50 top-1/2 left-1/2 max-h-[90vh] w-[800px] max-w-[95vw] -translate-x-1/2 -translate-y-1/2 overflow-y-auto p-6">
          <Theme>
            ...
            <Flex direction="column" gap="2">
              ...
              <Flex gap="3">
                <Button onClick={updateLabel} disabled={!text.trim()}>
                  Save
                </Button>
                {editMode && (
                  <Button onClick={deleteLabel} color="red">
                    Delete
                  </Button>
                )}
              </Flex>
