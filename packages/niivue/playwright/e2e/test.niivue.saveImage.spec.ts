...
test('niivue save drawing LAS', async ({ page }) => {
    ...
    let correctImage = correctImageBuffer.buffer.slice(correctImageBuffer.byteOffset, correctImageBuffer.byteOffset + correctImageBuffer.byteLength)
    const incorrectImageBuffer = fs.readFileSync('./tests/images/LAS_drawing_incorrect.nii.gz')
    let incorrectImage = incorrectImageBuffer.buffer.slice(incorrectImageBuffer.byteOffset, incorrectImageBuffer.byteOffset + incorrectImageBuffer.byteLength)
    const downloadedImageBuffer = fs.readFileSync(filePath)
    let downloadedImage = downloadedImageBuffer.buffer.slice(downloadedImageBuffer.byteOffset, downloadedImageBuffer.byteOffset + downloadedImageBuffer.byteLength)
