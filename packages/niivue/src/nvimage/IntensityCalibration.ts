...
export function calMinMax(nvImage: NVImage, vol: number = Number.POSITIVE_INFINITY, isBorder: boolean = true): number[] {
  ...
  if (!isBorder) {
    ...
    const borders = [
      Math.floor(borderFrac * nvImage.hdr.dims[1]),
      Math.floor(borderFrac * nvImage.hdr.dims[2]),
      Math.floor(borderFrac * nvImage.hdr.dims[3])
    ]
    const dims = [
      nvImage.hdr.dims[1] - 2 * borders[0],
      nvImage.hdr.dims[2] - 2 * borders[1],
      nvImage.hdr.dims[3] - 2 * borders[2]
    ]
  ...
  if (
    ...
    isFinite(nvImage.hdr.cal_min) &&
    isFinite(nvImage.hdr.cal_max) &&
    nvImage.hdr.cal_max > nvImage.hdr.cal_min
  ) {
