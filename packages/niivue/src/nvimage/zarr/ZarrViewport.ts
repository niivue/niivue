...
export class ZarrViewport {
  ...
  constructor(pyramidInfo: ZarrPyramidInfo, volumeDims: VirtualVolumeDimensions) {
    ...
    let bestLevel = this.findBestInitialLevel()
  ...
  private findBestInitialLevel(): number {
    for (let i = 0; i < this.pyramidInfo.levels.length; i++) {
      ...
      if (
        dims.width <= this.volumeDims.width &&
        dims.height <= this.volumeDims.height &&
        dims.depth <= this.volumeDims.depth
      ) {
        return i
      }
    }
  ...
  setState(newState: Partial<ZarrViewportState>): void {
    ...
    if (newState.centerZ !== undefined) this.state.centerZ = newState.centerZ
  ...
  getBaseScale(): number {
    const level0Dims = this.getLevel0Dimensions()
    return Math.min(
      this.volumeDims.width / level0Dims.width,
      this.volumeDims.height / level0Dims.height,
      this.volumeDims.depth / level0Dims.depth
    )
  }
  ...
  getVisibleChunks(): ChunkCoord[] {
    ...
    const endChunkX = Math.min(
      Math.ceil(levelDims.width / chunkDims.width),
      Math.ceil(region.maxX / chunkDims.width)
    )
    const endChunkY = Math.min(
      Math.ceil(levelDims.height / chunkDims.height),
      Math.ceil(region.maxY / chunkDims.height)
    )
    const endChunkZ = Math.min(
      Math.ceil(levelDims.depth / chunkDims.depth),
      Math.ceil(region.maxZ / chunkDims.depth)
    )
