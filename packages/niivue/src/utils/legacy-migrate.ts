...
function stabilizeNumber(n: number, decimals = 12): number {
  ...
  if (Math.abs(n - rounded) <= eps) return rounded
  return n
}

function stabilizeArray(arr: any[], decimals = 12): any[] {
  return arr.map((v) => {
    if (Array.isArray(v)) return stabilizeArray(v, decimals)
    if (isNumeric(v)) return stabilizeNumber(v, decimals)
    return v
  })
}
...
export function extractNumberArray(maybe: any): number[] | undefined {
  ...
  if (typeof maybe === 'object') {
    if (Array.isArray((maybe as any).data)) return (maybe as any).data.slice()
    if (maybe.buffer && Array.isArray(maybe.buffer.data)) return maybe.buffer.data.slice()
    // numeric-keyed object like { "0": v0, "1": v1, length: N }
    const numericKeys = Object.keys(maybe).filter(k => /^\d+$/.test(k)).map(k => Number(k)).sort((a,b)=>a-b)
    ...
    for (const candidateKey of ['_data', 'source', 'values', 'elements', 'items']) {
      if (Array.isArray((maybe as any)[candidateKey])) return (maybe as any)[candidateKey].slice()
    }
  }
  try {
...
export function normalizeMeshForRehydrate(meshIn: any): any {
  if (!meshIn || typeof meshIn !== 'object') return meshIn
  const m: any = { ...(meshIn || {}) }
  ...
  if ((m.pts === undefined || m.pts === null) && (m.vertices !== undefined || m.positions !== undefined || m.verts !== undefined)) {
    ...
    if (arr) m.pts = stabilizeArray(arr)
    ...
    m.layers = m.layers.map((layer: any) => {
      ...
      for (const k of ['global_min','global_max','cal_min','cal_max','cal_minNeg','cal_maxNeg']) {
        if (typeof lcopy[k] === 'string') {
          const v = lcopy[k]
          if (v === 'infinity') lcopy[k] = Infinity
          else if (v === '-infinity') lcopy[k] = -Infinity
          else if (v === 'NaN') lcopy[k] = NaN
          else {
            const n = Number(v)
            if (!Number.isNaN(n)) lcopy[k] = n
          }
        }
      }
      ...
      if (lcopy.values != null && !Array.isArray(lcopy.values)) {
        ...
        if (ev) lcopy.values = stabilizeArray(ev)
...
function normalizeMeshesString(meshesString: any): string | undefined {
  if (typeof meshesString !== 'string') return undefined
  const trimmed = meshesString.trim()
  if (trimmed.length === 0) return meshesString
  try {
    ...
    const looksLikeStructuredClone =
...
export function migrateLegacyDocument(input: DocumentData | any | undefined): DocumentData {
  if (!input || typeof input !== 'object') return input as DocumentData

  const doc: any = { ...(input || {}) }

  // normalize root-level legacy colormap names
  if ('colorMap' in doc && !('colormap' in doc)) doc.colormap = doc.colorMap
  if ('colorMapNegative' in doc && !('colormapNegative' in doc)) doc.colormapNegative = doc.colorMapNegative
  ...
  doc.imageOptionsArray = doc.imageOptionsArray.map((io:any) => {
    ...
    for (const k of ['cal_min','cal_max','cal_minNeg','cal_maxNeg']) {
      if (typeof copy[k] === 'string') {
        if (copy[k] === 'infinity') copy[k] = Infinity
        else if (copy[k] === '-infinity') copy[k] = -Infinity
        else if (copy[k] === 'NaN') copy[k] = NaN
        else {
  ...
  if (doc.labels && !Array.isArray(doc.labels)) {
    try { doc.labels = Array.from(doc.labels) } catch (_) { /* leave as-is */ }
  }
  if (doc.connectomes && !Array.isArray(doc.connectomes)) {
    try { doc.connectomes = Array.from(doc.connectomes) } catch (_) { /* leave as-is */ }
  }
  if (doc.encodedImageBlobs && !Array.isArray(doc.encodedImageBlobs)) {
    try { doc.encodedImageBlobs = Array.from(doc.encodedImageBlobs) } catch (_) { /* leave as-is */ }
  }
