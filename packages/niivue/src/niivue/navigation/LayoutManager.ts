...
export function calculateScreenFieldOfViewMM(params: ScreenFieldOfViewMMParams): vec3 {
  const {
    ...
    extentsMin,
    extentsMax,
    volumeObjectExtentsMin,
    volumeObjectExtentsMax,
    isSliceMM,
    forceSliceMM,
    getScreenFieldOfViewVox,
    swizzleVec3MM,
    axCorSag
  } = params
...
export function calculateXyMM2xyzMM(params: XyMM2xyzMMParams): number[] {
  ...
  const denom =
    (bSwizzled[0] - aSwizzled[0]) * (cSwizzled[1] - aSwizzled[1]) -
    (cSwizzled[0] - aSwizzled[0]) * (bSwizzled[1] - aSwizzled[1])
  let yMult =
    (bSwizzled[0] - aSwizzled[0]) * (cSwizzled[2] - aSwizzled[2]) -
    (cSwizzled[0] - aSwizzled[0]) * (bSwizzled[2] - aSwizzled[2])
  yMult /= denom
  let xMult =
    (bSwizzled[1] - aSwizzled[1]) * (cSwizzled[2] - aSwizzled[2]) -
    (cSwizzled[1] - aSwizzled[1]) * (bSwizzled[2] - aSwizzled[2])
  xMult /= denom
...
export function calculateBulletMarginWidth(params: CalculateBulletMarginWidthParams): number {
  ...
  const widestBulletScale =
    labels.length === 1
      ? (labels[0].style.bulletScale ?? 1)
      : (labels.reduce((a, b) => ((a.style.bulletScale ?? 1) > (b.style.bulletScale ?? 1) ? a : b)).style.bulletScale ??
        1)
 ...
 */
export function isPointInBoundsCSS(params: {
  x: number
  y: number
  boundsRegion: [number, number, number, number]
}): boolean {
 ...
 */
export function isCursorInBounds(params: {
  mouseX: number
  mouseY: number
  boundsRegion: [number, number, number, number]
}): boolean {
