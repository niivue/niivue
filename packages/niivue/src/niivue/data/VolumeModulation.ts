...
export function setupModulation(params: ModulationParams): ModulationResult {
  ...
  if (
    ...
    overlayItem.modulationImage >= volumes.length
  ) {
  ...
  if (
    isFinite(volumes[overlayItem.modulationImage].cal_minNeg) &&
    isFinite(volumes[overlayItem.modulationImage].cal_maxNeg)
  ) {
