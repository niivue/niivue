...
export function sceneExtentsMinMax(params: SceneExtentsParams, isSliceMM = true): vec3[] {
  ...
  if (volumes.length > 0) {
    ...
    if (!isSliceMM) {
      mn = vec3.fromValues(
        volumes[0].extentsMinOrtho![0],
        volumes[0].extentsMinOrtho![1],
        volumes[0].extentsMinOrtho![2]
      )
      mx = vec3.fromValues(
        volumes[0].extentsMaxOrtho![0],
        volumes[0].extentsMaxOrtho![1],
        volumes[0].extentsMaxOrtho![2]
      )
    }
  }
