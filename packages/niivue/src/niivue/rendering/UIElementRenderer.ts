 ...
 */
export function calculateTextBelowPosition(
  params: TextPositionParams,
 ...
 * @returns Adjusted [x, y] position and adjusted scale
 */
export function calculateTextAbovePosition(
  params: TextPositionParams,
  getTextWidth: (size: number, str: string) => number
): { x: number; y: number; scale: number } {
  const { xy, str, fontPx, canvasWidth } = params
 ...
 */
export function calculateDottedLineSegments(
  startXYendXY: number[],
  fontPx: number,
  scale: number
): DottedLineSegment[] {
 ...
 */
export function calculateOrientationCubePosition(
