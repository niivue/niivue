...
export type {
  LoaderRegistry, CustomLoader, GetFileExtOptions, RegisterLoaderParams, MeshLoaderResult
} from '@/niivue/data/FileLoader'
...
if (perm[0] === 1 && perm[1] === 2 && perm[2] === 3) {
    ...
    const promises = meshOptions.map(async (meshItem) => {
      ...
      if (this.loaders[ext]) {
        ...
        const {
          ...
          scalars = null,
          colormapLabel = null
        } = await this.loaders[ext].loader(itemToLoad)
  ...
  drawMeasurementTool(startXYendXY: number[], isDrawText: boolean = true): void {
    ...
    const extendTo = (x0: number, y0: number, x1: number, y1: number, distance: number): {
      origin: number[];
      terminus: number[]
    } => {
