...
test('parse_xml_gifti_file_ok', async () => {
...
test('parse_xml_gifti_file', async () => {
	let scalars = await page.evaluate(async () => {
		let xmlFile = `<?xml version="1.0" encoding="UTF-8"?>
		<!DOCTYPE GIFTI SYSTEM "http://www.nitrc.org/frs/download.php/115/gifti.dtd">
		<GIFTI Version="1.0" NumberOfDataArrays="1"><MetaData /><LabelTable /><DataArray Intent="NIFTI_INTENT_ZSCORE" DataType="NIFTI_TYPE_INT64" ArrayIndexingOrder="RowMajorOrder" Dimensionality="1" Encoding="GZipBase64Binary" Endian="LittleEndian" ExternalFileName="" ExternalFileOffset="0" Dim0="3"><MetaData /><CoordinateSystemTransformMatrix><DataSpace>NIFTI_XFORM_UNKNOWN</DataSpace><TransformedSpace>NIFTI_XFORM_UNKNOWN</TransformedSpace><MatrixData>  1.000000   0.000000   0.000000   0.000000
		0.000000   1.000000   0.000000   0.000000
		0.000000   0.000000   1.000000   0.000000
		0.000000   0.000000   0.000000   1.000000</MatrixData></CoordinateSystemTransformMatrix><Data>eJxjZIAAJijNDKUBAGgABw==</Data></DataArray></GIFTI>`;
		function base64ToArrayBuffer(base64) {
			var binary_string = atob(base64);
			var len = binary_string.length;
			var bytes = new Uint8Array(len);
		...
		let buffer = base64ToArrayBuffer(btoa(xmlFile));
		let scalars = niivue.NVMesh.readGII(buffer);
		return scalars;
	})
