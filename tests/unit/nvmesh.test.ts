...
import { NVMesh } from "../../src/niivue/index.js"; // note the js extension
...
test("nvmesh parses xml gifti file with new line", () => {
  const xmlFile = `
...
const obj = NVMesh.readGII(buffer)
expect(Object.values(obj.scalars).length).toBe(3)
});

test("nvmesh parses xml gifti file with no new line", () => {
  const xmlFile = `
