 ...
 * @property {number[]} point - point in mm (X,Y,Z)
 ...
 */
export class NVConnectomeNode {
  constructor(name, point, colorValue, sizeValue) {
    this.name = name;
    this.point = point;
...
export class NVConnectome extends NVMesh {
  ...
  updateConnectome(gl) {
    ...
    for (let i = 0; i < nNode; i++) {
      ...
      console.log("pt for node", pt);
      console.log("node", this.nodes[i]);
      console.log("pts after make sphere", pts);
    }
    ...
    console.log("pts for extents", pts);
