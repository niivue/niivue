...
export class NVMeshLoaders {
  ...
  static async readTRX(buffer) {
    ...
    if (noff === 0 || npt === 0) {
      utiltiesLogger.error(
        "Failure reading TRX format (no offsets or points)."
      );
      return;
    }
    if (isOverflowUint64)
  ...
  static readLayer(
    ...
    if (ext === "MZ3") layer.values = NVMeshLoaders.readMZ3(buffer, n_vert);
      ...
      if (!isReadColortables)
        ...
        let obj = NVMeshLoaders.readANNOT(buffer, n_vert, true);
        if ("scalars" in obj) {
      ...
      let obj = NVMeshLoaders.readGII(buffer, n_vert);
      ...
      if (!isReadColortables)
        ...
        let obj = NVMeshLoaders.readMGH(buffer, n_vert, true);
        if ("scalars" in obj) {
          layer.values = obj.scalars;
          layer.colormapLabel = obj.colormapLabel;
        } //unable to decode colormapLabel
  ...
  static readMGH(buffer, n_vert = 0, isReadColortables = false) {
    ...
    if (reader.getUint8(0) === 31 && reader.getUint8(1) === 139) {
      let decompressed = fflate.decompressSync(new Uint8Array(buffer));
    ...
    while (tag.endPos < len && tag.name.length > 1) {
      ...
      if (tag.name.trim() === "MD") {
        ...
        if (line.includes("VolGeom") && line.includes("CDATA[")) {
          ...
          if (e < 0) continue;
          FreeSurferTranlate[e] = parseFloat(
            readBracketTag("<Value><![CDATA[")
          );
        }
      }
