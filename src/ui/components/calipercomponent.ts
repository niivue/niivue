...
export class CaliperComponent extends BaseUIComponent {
    ...
    constructor(pointA: Vec2, pointB: Vec2, length: number, units: string, font: NVFont, textColor: Color = [1, 0, 0, 1], lineColor: Color = [0, 0, 0, 1], scale: number = 1.0) {
        super()
        this.pointA = pointA
        this.pointB = pointB
        this.length = length
    ...
    draw(renderer: NVRenderer): void {
        ...
        if (angle > Math.PI / 2 || angle < -Math.PI / 2) {
            angle += Math.PI
            textPosition = [
                midPoint[0] - (textWidth / 2) * Math.cos(angle) - (textHeight / 2 + 10) * Math.sin(angle) - 10 * Math.sin(angle),
        ...
        renderer.drawLine([parallelPointA[0], parallelPointA[1], this.pointA[0], this.pointA[1]], 1, this.lineColor, LineTerminator.ARROW)
        renderer.drawLine([parallelPointB[0], parallelPointB[1], this.pointB[0], this.pointB[1]], 1, this.lineColor, LineTerminator.ARROW)
    }
