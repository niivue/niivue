...
test('niivue draw 3D crosshair', async ({ page }) => {
    const nvols = await page.evaluate(async (testOptions) => {
        const nv = new Niivue(Object.assign(Object.assign({}, testOptions), { show3Dcrosshair: true }));
        await nv.attachTo('gl');
        // load one volume object in an array
        const volumeList = [
            {
...
test('niivue draw 3D no crosshair', async ({ page }) => {
    const nvols = await page.evaluate(async (testOptions) => {
        const nv = new Niivue(Object.assign(Object.assign({}, testOptions), { show3Dcrosshair: false }));
        await nv.attachTo('gl');
        // load one volume object in an array
        const volumeList = [
            {
