...
test('niivue loadMeshes MZ3', async ({ page }) => {
    const nmeshes = await page.evaluate(async (testOptions) => {
        ...
        const layers = [
            Object.assign(Object.assign({}, testOptions.defaultLayerOptions), { url: './images/mz3/11ScalarMesh.mz3', colormap: 'actc' })
        ];
        // const layers = { ...defaultLayerOptions, ...layersList }
        await nv.loadMeshes([{ url: './images/mz3/3Mesh.mz3', layers }]);
        return nv.meshes.length;
    }, Object.assign(Object.assign({}, TEST_OPTIONS), { defaultLayerOptions }));
...
test('niivue loadMeshes MZ3 with double layer', async ({ page }) => {
    const nmeshes = await page.evaluate(async (testOptions) => {
        ...
        await nv.attachTo('gl');
        const layersList = [
            Object.assign(Object.assign({}, testOptions.defaultLayerOptions), { url: './images/mz3/16DoubleOverlay_5124x2.mz3', colormap: 'actc' })
        ];
...
for (const file of meshFormats) {
    test(`niivue loadMeshes for file format ${file.fileName}`, async ({ page }) => {
        ...
        }, Object.assign(Object.assign({}, TEST_OPTIONS), { file }));
