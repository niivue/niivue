...
;(0, test_1.test)('niivue removeVolume', async ({ page }) => {
...
;(0, test_1.test)('niivue removeVolume remove all volumes', async ({ page }) => {
  const nvols = await page.evaluate(async (testOptions) => {
    const nv = new index_1.Niivue(testOptions)
    await nv.attachTo('gl')
    // load one volume object in an array
...
// # sourceMappingURL=test.niivue.removeVolume.spec.js.map
