...
test_1.test.beforeEach(async ({ page }) => {
  ...
  await page.goto(helpers_1.httpServerAddressFlexbox)
  ...
  await (0, test_1.expect)(page.getByText('bottom of page')).toBeInViewport()
})
// skipped because I don't have a solution for this atm.
test_1.test.skip('canvas height decreases when viewport height decreases', async ({ page }) => {
...
;(0, test_1.test)(
  'canvas height increases by *close to the* same amount as viewport when viewport is enlarged',
  async ({ page }) => {
    await (0, test_1.expect)(page.getByText('bottom of page')).toBeInViewport()
    const originalBox = await page.locator('#gl').boundingBox()
    if (originalBox === null) {
...
// # sourceMappingURL=test.niivue.flexboxResize.spec.js.map
