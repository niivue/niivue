...
;(0, test_1.test)('niivue setSliceType axial', async ({ page }) => {
...
;(0, test_1.test)('niivue setSliceType coronal', async ({ page }) => {
  await page.evaluate(async (testOptions) => {
    const nv = new index_1.Niivue(testOptions)
    await nv.attachTo('gl')
    // load one volume object in an array
...
;(0, test_1.test)('niivue setSliceType sagittal', async ({ page }) => {
  await page.evaluate(async (testOptions) => {
    const nv = new index_1.Niivue(testOptions)
    await nv.attachTo('gl')
    // load one volume object in an array
...
// # sourceMappingURL=test.niivue.setSliceType.spec.js.map
